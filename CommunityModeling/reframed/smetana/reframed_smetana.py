...
class smetana():
    ...
    def __init__(self, cobra_models:Union[list, tuple, set]):
        # convert COBRA model into ReFramed model
        self.model = Community('SMETANA', [FBAHelper.get_reframed_model(model) for model in cobra_models])
    
    def sc_score(self, community=None, environment=None, min_growth=0.1, n_solutions=100, abstol=1e-6):
        """
    ...
    def mu_score(self, community=None, environment=None, min_mol_weight=False, min_growth=0.1, max_uptake=10.0,
        ...
        for org_id in community.organisms:
            ...
            medium_list, sols = minimal_medium(
                community.merged, exchange_reactions=list(exchange_rxns.keys()), min_mass_weight=min_mol_weight, 
                min_growth=min_growth, n_solutions=n_solutions, max_uptake=max_uptake, validate=validate, 
                abstol=abstol, solver=solver, warnings=False)
    ...
    def mp_score(self, community=None, environment=None, abstol=1e-3):
        """
        ...
        for org_id, exchange_rxns in community.organisms_exchange_reactions.items():
            ...
            for r_id in remaining:
                ...
                if sol.status.value == 'Optimal' and sol.fobj > abstol:
                    scores[org_id][cnm.original_metabolite] = 1
                else:
    ...
    def smetana_score(self, community=None, environment=None, min_mol_weight=False, min_growth=0.1, max_uptake=10.0, 
                      validate=False, n_solutions=100, verbose=True, abstol=1e-6):  # the sum of all interspecies dependencies under a given nutritional environment
