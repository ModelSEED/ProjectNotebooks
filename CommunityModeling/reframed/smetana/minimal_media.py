...
class MinMedia:
    ...
    @staticmethod
    def minimal_medium(model, media:dict=None, min_objective_value: float = 0.1, exports: bool = False, 
                       minimize_components: Union[bool, int] = False, open_exchanges: bool = False
                       ) -> dict:
        ...
        """
        media = media or model.medium
        variables = []
        for cpd in media:
            variables.append(OptlangHelper.add_variables(cpd, (0,1), "binary"))
        objective = ("Test Helper Dictionary", [var.name for var in variables], "min")
        
        
        
        
        
        ######################### ORIGINAL COBRAPY FORMULATION ##############################
        exchange_rxns = find_boundary_types(model, "exchange")
