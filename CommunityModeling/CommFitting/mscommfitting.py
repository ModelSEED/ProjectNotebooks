...
class MSCommFitting():   # explicit typing for cython
            ...
            for time in parsed_df[1]:  # 27
                ...
                for r_index, trial in enumerate(parsed_df[0]):  # 66
                    ...
                    constraints.extend([self.constraints[signal+'__bioc'][time][trial], 
                                        ...
                                        self.constraints['dbc_'+signal+'_'+strain][time][trial]])
    ...
    def compute(self, graphed_trials:list = []):
        ...
        for variable, value in self.problem.primal_values.items():
            ...
            if not basename in self.values:
                self.values[basename]:dict = {}
                if not time in self.values[basename]:
                    self.values[basename][time]:dict = {}
            self.values[basename][time][trial] = value
            
        for trial in graphed_trials:
