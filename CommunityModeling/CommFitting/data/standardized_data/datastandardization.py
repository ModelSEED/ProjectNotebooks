...
class GrowthData:
    ...
    def load_data(base_media, community_members, solver, signal_csv_paths,
        ...
        raw_data = _spreadsheet_extension_load(signal_csv_paths['path'])
...
class BiologData:
    ...
    @staticmethod
    def process(data_path, trial_conditions_path, significant_deviation):
        (zipped_output, data_timestep_hr, simulation_time, signal_species, dataframes, trials
        ) = BiologData.load_data(data_path, significant_deviation)
        experimental_metadata, standardized_carbon_conc, trial_name_conversion = BiologData.metadata(
            trial_conditions_path, community_members, species_abundances, carbon_conc_series,
            species_identities_rows, findDate(signal_csv_paths["path"])
        )
        growth_dfs = BiologData.data_process(dataframes, trial_name_conversion)
        return (experimental_metadata, growth_dfs, fluxes_df, standardized_carbon_conc, signal_species,
                trial_name_conversion, species_phenos_df, np.mean(data_timestep_hr), simulation_time, media_conc)

    @staticmethod
    def load_data(data_path, significant_deviation=2):
        # import and parse the raw CSV data
        data_timestep_hr = []
        dataframes, signal_species = {}, {}
        zipped_output = [data_path['path']]
        raw_data = _spreadsheet_extension_load(data_path['path'])
        for org_sheet, name in data_path.items():
            if org_sheet == 'path':
                continue
            sheet = org_sheet.replace(' ', '_')
            if sheet not in dataframes:
                dataframes[sheet] = _spreadsheet_extension_parse(
                    data_path['path'], raw_data, org_sheet)
                dataframes[sheet].columns = dataframes[sheet].iloc[6]
                dataframes[sheet].drop(dataframes[sheet].index[:7], inplace=True)
            # parse the DataFrame for values
            signal_species[sheet] = name
        ...
        # differentiate the phenotypes for each species
        trials = set(chain.from_iterable([list(df.index) for df, times in dataframes.values()]))
        return (zipped_output, data_timestep_hr, simulation_time, signal_species,
                dataframes, trials)
    ...
    def metadata(trial_conditions_path, culture, date):
        ...
        constructed_experiments["short_code"] = [f"{experiment_prefix}{x+1}" for x in list(range(row_num*column_num))]
        ...
        for row in range(1, row_num+1):
            ...
            for col in range(1, column_num+1):
                ## construct the columns of information
                experiment_id = trial_conditions[trial_letter+str(col)]
                experiment_ids.append(experiment_id)
                trial_name_conversion[trial_letter][str(col)] = experiment_prefix+str(count)
                count += 1

        # convert the variable concentrations to short codes
        standardized_carbon_conc = {}
        for met, conc in trial_conditions_path["rows"].items():
            standardized_carbon_conc[met] = {}
            for row, val in conc.items():
                standardized_carbon_conc[met].update({short_code:val for short_code in trial_name_conversion[row].values()})

        # add columns to the exported dataframe
        constructed_experiments.insert(0, "trial_IDs", experiment_ids)
        constructed_experiments["strains"] = [culture] * (column_num*row_num)
        constructed_experiments["date"] = [date] * (column_num*row_num)
        constructed_experiments.to_csv("growth_metadata.csv")
        return constructed_experiments, standardized_carbon_conc, trial_name_conversion
