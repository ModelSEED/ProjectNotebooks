...
class MSCommFitting():   # explicit typing for cython
    ...
    def __init__(self, 
        ...
        if phenotypes_csv_path:
            ...
            # import KBase
            # if not kbase_token and not os.path.exists('~/.kbase/token'):
    ...
    def define_problem(self, conversion_rates={'cvt': 0, 'cvf': 0}, 
        ...
        trial: str; time: str; name: str; strain: str; met: str; growth_stoich: cython.float = 0; variable_names:str = ''
        ...
        for signal, parsed_df in self.dataframes.items():  # 3
            ...
            self.variables[signal+'__bio']:dict = {}; self.variables[signal+'__diff']:dict = {}
            ...
            for r_index, trial in enumerate(parsed_df[0]):  # 66
                self.variables[signal+'__bio'][trial]:dict = {}; self.variables[signal+'__diff'][trial]:dict = {}
                ...
                for time in parsed_df[1]:  # 27
                    ...
                    self.variables[signal+'__diff'][trial][time] = Variable(_variable_name(signal, '__diff', trial, time), lb=-100, ub=100)
                    ...
                    self.constraints[signal+'__diffc'][trial][time] = Constraint(
                        self.variables[signal+'__bio'][trial][time]-signal_sum - self.variables[signal+'__diff'][trial][time], 
                        name=_constraint_name(signal, '__diffc', trial, time), lb=0, ub=0)

                    obj_coef[self.variables[signal+'__diff'][trial][time]] = 1                            
                    variables.extend([conversion, self.variables[signal+'__bio'][trial][time], 
                                      self.variables[signal+'__diff'][trial][time]])
    ...
    def _export(self, solution, print_conditions, print_lp):
        with open('solution.json', 'w') as sol_out:
            json.dump(solution, sol_out, indent = 4)
        zipped_output = ['solution.json']
        if print_conditions:
            DataFrame(self.parameters).to_csv('parameters.csv')
            zipped_output.append('constraints.csv')
        if print_lp:
    ...
    def compute(self, print_conditions: bool = True, print_lp: bool = True):
        self.solution = self.problem.optimize()
